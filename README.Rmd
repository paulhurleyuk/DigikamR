...
ggplot(data=subset(ImgMerge, focalLength<60),
       aes(x=as.POSIXct(creationDate), y=focalLength, colour=model))+
         geom_point()

ggplot(data=ImgMerge, aes(x=focalLength))+
  geom_histogram(binwidth=5, aes(colour=as.factor(model)))+
  facet_grid(model~.)
...
ggplot(data=subset(ImgMerge, model=="NIKON D5000"),
       aes(x=as.POSIXct(creationDate), y=focalLength))+
         geom_point()

ggplot(data=subset(ImgMerge, model=="NIKON D5000" & focalLength<60),
       aes(x=as.POSIXct(creationDate), y=focalLength))+
